...
import com.tc.midterm_api_mvvm_firebase.ui.satscorelist.SatScoreDetailScreen
import com.tc.midterm_api_mvvm_firebase.ui.schoollist.SchoolScreen
...
fun AppNavigation() {
    ...
    val targetScreen = rememberSaveable { mutableStateOf(Screens.SchoolList) }
    ...
    Scaffold(
        ...
        NavHost(
            ...
            startDestination = Screens.SchoolList.route
        ) {
            composable(Screens.SchoolList.route) {
                targetScreen.value = Screens.SchoolList
                SchoolScreen(navController = navController)
            }
            composable("${Screens.SchoolList.route}/{dbn}") { backStackEntry ->
                targetScreen.value = Screens.SchoolList
                SatScoreDetailScreen(
...
fun Toolbar(
    ...
    ) = if (targetScreen != Screens.SchoolList) {
    TopAppBar(
